...
class TypeChecker:
    ...
    def infer(self, ctx: LocalContext, e: Expr) -> Expr:
        ...
        match e.kind:
            ...
            case Expr.Lit(Literal.NatVal(_)):
                # 自然数字面量的类型是 Nat
                # 简化处理：返回常量 Nat
            ...
            case Expr.Lit(Literal.StrVal(_)):
                # 字符串字面量的类型是 String
                from .name import mk_name
    ...
    def is_def_eq_core(self, t1: Expr, t2: Expr) -> bool:
        ...
        match (t1.kind, t2.kind):
            ...
            case (Expr.Lam(_, d1, b1, _), Expr.Lam(_, d2, b2, _)):
                ...
                return self.is_def_eq(d1, d2) and self.is_def_eq(b1, b2)
            ...
            case (Expr.App(f1, a1), Expr.App(f2, a2)):
                # 应用等价：函数部分等价 且 参数等价
                # f a ≡ f' a'  当  f ≡ f' 且 a ≡ a'
                return self.is_def_eq(f1, f2) and self.is_def_eq(a1, a2)
            ...
            case (Expr.LetE(_, d1, v1, b1), Expr.LetE(_, d2, v2, b2)):
                return (
                    self.is_def_eq(d1, d2)
                    and self.is_def_eq(v1, v2)
                    and self.is_def_eq(b1, b2)
                )
    ...
    def _reduce_level(self, level: Level) -> Level:
        ...
        match level.kind:
            ...
            case Level.IMax(_, _):
                ...
                if not isinstance(rl2.kind, Level.Zero) and not isinstance(
                    rl2.kind, Level.Param
                ):
                    return Level.max_level(rl1, rl2)
