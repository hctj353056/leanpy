...
from .expr import Expr, BinderInfo, app, lam, arrow, forall_
from .environment import (
    Environment, LocalContext, LocalDecl, 
    MetavarContext, MetavarDecl,
    AxiomVal, DefnVal, InductVal, CtorVal
)
...
from .parser import parse_expr, parse_decl
from .tactic import (
    Goal, ProofState, Tactic, Intro, Apply, Exact, 
    Rewrite, Assumption,
    by_tactics, start_proof, finish_proof, get_proof_term,
    reset_mvar_counter
)
...
def setup_nat_environment() -> Environment:
    ...
    env = env.add(mk_name("Nat", "add"), AxiomVal(name=mk_name("Nat", "add"), type=add_type))
...
def example_and_elim_left():
    ...
    print(f"合取消去（概念）: 从 Prod A B 提取 A")
